...
def fetch_rows(client: NaverSearchAdClient, keywords: list[str], delay: float) -> list[dict]:
    rows: list[dict] = []
...
def main() -> None:
    ...
    keywords = sorted({str(v).strip() for v in df[keyword_col].dropna() if str(v).strip()})
    if not keywords:
        raise SystemExit(f"'{keyword_col}' 컬럼에서 키워드를 찾지 못했습니다.")

    print(f"'{keyword_col}' 컬럼에서 키워드 {len(keywords)}개를 수집했습니다.")

    client = NaverSearchAdClient(settings)
    rows = fetch_rows(client, keywords, args.delay)
